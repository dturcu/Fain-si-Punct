...
export async function createOrder(
  ...
  if (itemsErr) {
    // Best-effort rollback — if the items insert fails we should not leave
    // an item-less order behind. If rollback itself fails we still surface
    // the original error so callers can act.
    const { error: rollbackErr } = await supabaseAdmin
      .from('orders')
      .delete()
      .eq('id', order.id)
    if (rollbackErr) itemsErr.rollbackError = rollbackErr
    throw itemsErr
  }
