...
export async function getUserByEmail(email) {
  ...
  // Include password hash for auth verification
...
export async function createOrder(userId, items, total, customer, shippingAddress, orderNumber, paymentMethod = 'card', guestSessionId = null) {
  // Map payment method to DB enum value
  const dbPaymentMethod = paymentMethod === 'ramburs' ? null : (paymentMethod === 'card' || paymentMethod === 'revolut') ? 'stripe' : 'paypal'
  ...
  if (guestSessionId) {
    orderRow.guest_session_id = guestSessionId
  }
...
export async function addToCart(userId, productId, productName, productPrice, productImage, quantity, variantId = null, variantLabel = null) {
...
export async function addToGuestCart(guestSessionId, productId, productName, productPrice, productImage, quantity, variantId = null, variantLabel = null) {
...
export async function migrateGuestToUser(guestSessionId, userId) {
  // Migrate guest cart to user (if user doesn't already have one)
  ...
  if (guestCart && !existingUserCart) {
    ...
    // Merge: move guest cart items into user's cart, then delete guest cart
    await supabaseAdmin
      .from('cart_items')
